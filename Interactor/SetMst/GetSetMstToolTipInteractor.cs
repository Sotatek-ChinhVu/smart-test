...
public class GetSetMstToolTipInteractor : IGetSetMstToolTipInputPort
{
    ...
    public GetSetMstToolTipOutputData Handle(GetSetMstToolTipInputData inputData)
    {
        try
        {
            ...
            return !(tooltip.DictionaryOrders.Count == 0 && tooltip.ListByomeis.Count == 0 && tooltip.ListKarteNames.Count == 0) ? new GetSetMstToolTipOutputData(tooltip, GetSetMstToolTipStatus.Successed) : new GetSetMstToolTipOutputData(new(), GetSetMstToolTipStatus.NoData);
        }
        finally
        {
