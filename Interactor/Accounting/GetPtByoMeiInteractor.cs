...
namespace Interactor.Accounting
{
    public class GetPtByoMeiInteractor : IGetPtByoMeiInputPort
    {
        ...
        private List<PtDiseaseDto> ConvertToPtDiseaseDto(List<PtDiseaseModel> models)
        {
            ...
            {
                ptDiseaseDto.Add(new PtDiseaseDto(item.PtId, item.ByomeiCd, item.SeqNo, item.SyubyoKbn, item.SikkanKbn, item.FullByomei, item.StartDate, item.TenKiBinding, item.TenkiDate, item.HokenId));
            }
            return ptDiseaseDto;
        }
    }
