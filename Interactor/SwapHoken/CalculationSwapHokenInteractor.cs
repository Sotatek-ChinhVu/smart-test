...
namespace Interactor.SwapHoken
{
    public class CalculationSwapHokenInteractor : ICalculationSwapHokenInputPort
    {
        ...
        private StringBuilder GetErrorTextAfterCheck(int hpId, int seikyuYm, List<long> ptIdList)
        {
            List<SystemConfModel> systemConfList = _systemConfRepository.GetAllSystemConfig(hpId);
            List<ReceRecalculationModel> receRecalculationList = _receiptRepository.GetReceRecalculationList(hpId, seikyuYm, ptIdList);
            List<ReceInfForCheckErrSwapHokenModel> receInfModels = _receiptRepository.GetReceInforCheckErrForCalculateSwapHoken(hpId, ptIdList, seikyuYm);

            List<string> errors = new List<string>();
            foreach (var receInfModel in receInfModels)
            {
                var sinKouiCounts = _receiptRepository.GetSinKouiCountList(hpId, receInfModel.SinYm, receInfModel.PtId, receInfModel.HokenId);
                foreach (var sinKouiCount in sinKouiCounts)
                {
                    foreach (var sinKouiDetailModel in sinKouiCount.SinKouiDetailModels)
                    {
                        ...
                        {
                            errors.Add(string.Format("    {0}/{1} ID:{2} [{3}] {4}", seikyuYm / 100, seikyuYm % 100, receInfModel.PtNum, sinKouiDetailModel.ItemCd, sinKouiDetailModel.ItemName));
                        }
                    }
                }
            }
