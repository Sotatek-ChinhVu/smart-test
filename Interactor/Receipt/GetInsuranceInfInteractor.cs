...
namespace Interactor.Receipt
{
    public class GetInsuranceInfInteractor : IGetInsuranceInfInputPort
    {
        ...
        private List<InsuranceInfDto> ConvertToInsuranceInfDto(ReceInfModelDto receInf)
        {
            ...
            {
                insuranceInfs.Add(new InsuranceInfDto(item.HokenId, item.InsuranceName, item.HokenKbn, item.Nissu, item.Tensu, item.IchibuFutan, item.PtFutan, item.EdaNo, item.Kigo, item.Bango,
