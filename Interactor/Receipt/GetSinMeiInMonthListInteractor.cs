...
public class GetSinMeiInMonthListInteractor : IGetSinMeiInMonthListInputPort
{
    private readonly ICalculateService _calculateService;
    private readonly IMstItemRepository _mstItemRepository;

    public GetSinMeiInMonthListInteractor(ICalculateService calculateService, IMstItemRepository mstItemRepository)
    {
        _calculateService = calculateService;
        _mstItemRepository = mstItemRepository;
    }

    public GetSinMeiInMonthListOutputData Handle(GetSinMeiInMonthListInputData inputData)
    {
        try
        {
            var sinMeiInputData = new GetSinMeiDtoInputData(inputData.HpId, inputData.PtId, inputData.SinYm, inputData.SeikyuYm, inputData.HokenId, SinMeiModeConst.ReceCheck);
            var sinMei = GetSinMei(sinMeiInputData);
            var holiday = GetHolidayMst(inputData.HpId, inputData.SinYm);

            return new GetSinMeiInMonthListOutputData(sinMei, holiday, GetSinMeiInMonthListStatus.Successed);
        }
        finally
        {
    ...
    {
        var sinMeiViewModelDto = _calculateService.GetSinMeiInMonthList(sinMeiInputData);
