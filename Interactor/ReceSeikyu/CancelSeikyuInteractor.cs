...
public class CancelSeikyuInteractor : ICancelSeikyuInputPort
{
    ...
    public CancelSeikyuOutputData Handle(CancelSeikyuInputData inputData)
    {
        try
        {
            ...
            _calculateRepository.ReceFutanCalculateMain(new ReceCalculateRequest(new List<long>() { inputData.PtId }, inputData.SeikyuYm, string.Empty, string.Empty), CancellationToken.None);
