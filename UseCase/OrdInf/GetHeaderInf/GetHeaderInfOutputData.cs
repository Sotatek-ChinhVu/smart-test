...
namespace UseCase.OrdInfs.GetHeaderInf
{
    ...
    {
        public GetHeaderInfOutputData(int syosaiKbn, int jikanKbn, int hokenPid, int santeiKbn, int tantoId, int kaId, string uketukeTime, string sinStartTime, string sinEndTime, OrdInfModel odrInfs, GetHeaderInfStatus status)
        {
            SyosaiKbn = syosaiKbn;
            JikanKbn = jikanKbn;
            HokenPid = hokenPid;
