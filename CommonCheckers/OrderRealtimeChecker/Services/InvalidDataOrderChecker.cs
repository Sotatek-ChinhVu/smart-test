...
namespace CommonCheckers.OrderRealtimeChecker.Services
{
        ...
        where TOdrDetail : class, IOdrInfDetailModel
    {
        public override UnitCheckerResult<TOdrInf, TOdrDetail> HandleCheckOrder(UnitCheckerResult<TOdrInf, TOdrDetail> unitCheckerResult)
        {
            ...
            foreach (var detail in checkingOrder.OdrInfDetailModels)
            {
                ...
                if (!string.IsNullOrEmpty(detail.DisplayedUnit))
                {
                    ...
                    else if (detail.ItemCd == ItemCdConst.Con_Refill && detail.Suryo > SystemGenerationConfCommon.Instance.RefillSetting(unitCheckerResult.Sinday))
                    {
                        InvalidDataOrder error = new InvalidDataOrder()
                        {
                            ErrorType = ErrorType.RefillQuantityLimit,
                            ItemName = "'" + detail.ItemName + "'の数量は" + SystemGenerationConfCommon.Instance.RefillSetting(unitCheckerResult.Sinday) + "以下を入力してください。"
                        };
                        checkedResult.Add(error);
                    }
                }
                ...
                {
                    if (detail.Suryo.AsString().Length > 9)
                    {
                        InvalidDataOrder error = new InvalidDataOrder()
                        {
            ...
            if (checkingOrder.OdrKouiKbn >= 20 && checkingOrder.OdrKouiKbn <= 23 && checkingOrder.OdrInfDetailModels.Any(d => d.IsDrug))
            {
                var usage = checkingOrder.OdrInfDetailModels.FirstOrDefault(d => d.IsUsage);
                if (usage == null)
                {
                    InvalidDataOrder error = new InvalidDataOrder()
                    {
                ...
                {
                    //Check Bukantu items
                    var bunkatuItem = checkingOrder.OdrInfDetailModels.Where(item => item.ItemCd == ItemCdConst.Con_TouyakuOrSiBunkatu).FirstOrDefault();
                    if (bunkatuItem != null)
                    {
            ...
            if (checkingOrder.IsInjection && checkingOrder.OdrInfDetailModels.Any(d => d.IsInjection))
            {
                var usage = checkingOrder.OdrInfDetailModels.FirstOrDefault(d => d.IsUsage);
                if (usage == null)
                {
