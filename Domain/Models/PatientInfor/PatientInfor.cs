...
namespace Domain.Models.PatientInfor
{
    ...
    {
        public PatientInfor(HpId hpId, PtId ptId, ReferenceNo referenceNo, SeqNo seqNo, PtNum ptNum, string kanaName, string name, int sex, int birthday, int limitConsFlg, int isDead, int deathDate, string? homePost, string? homeAddress1, string? homeAddress2, string? tel1, string? tel2, string? mail, string? setanusi, string? zokugara, string? job, string? renrakuName, string? renrakuPost, string? renrakuAddress1, string? renrakuAddress2, string? renrakuTel, string? renrakuMemo, string? officeName, string? officePost, string? officeAddress1, string? officeAddress2, string? officeTel, string? officeMemo, int isRyosyoDetail, int primaryDoctor, int isTester, int mainHokenPid)
        {
            HpId = hpId;
            PtId = ptId;
