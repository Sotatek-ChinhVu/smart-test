...
public class ReceiptRepository : RepositoryBase, IReceiptRepository
{
    ...
    public bool SaveListSyobyoKeika(int hpId, int userId, List<SyobyoKeikaModel> listSyoukiInf)
    {
        var listSyobyoKeikaUpdate = listSyoukiInf.Where(item => item.SeqNo > 0).ToList();
        ...
        var listSyobyoKeikaAddNew = listSyoukiInf.Where(item => item.SeqNo == 0 && !item.IsDeleted)
