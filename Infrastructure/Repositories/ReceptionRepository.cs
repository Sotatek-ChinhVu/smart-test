...
namespace Infrastructure.Repositories
{
    public class ReceptionRepository : IReceptionRepository
    {
        ...
        public List<ReceptionRowModel> GetList(int hpId, long ptId, int sinDate)
        {
            return GetReceptionRowModels(hpId, ptId, sinDate);
        }
        ...
        private List<ReceptionRowModel> GetReceptionRowModels(int hpId, long ptId, int sinDate)
        {
            // 1. Prepare all the necessary collections for the join operation
            // Raiin (Reception)
            var raiinInfs = _tenantDataContext.RaiinInfs.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId && x.PtId == ptId && x.SinDate >= sinDate && x.IsYoyaku == 1);
            var raiinCmtInfs = _tenantDataContext.RaiinCmtInfs.Where(x => x.IsDelete == DeleteTypes.None && x.HpId == hpId && x.PtId == ptId && x.SinDate >= sinDate && x.CmtKbn == 1);
            var raiinKbnInfs = _tenantDataContext.RaiinKbnInfs.Where(x => x.IsDelete == DeleteTypes.None && x.HpId == hpId);
            var raiinKbnDetails = _tenantDataContext.RaiinKbnDetails.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId);
            // Pt (Patient)
            var ptInfs = _tenantDataContext.PtInfs.Where(x => x.IsDelete == DeleteTypes.None && x.HpId == hpId);
            var ptCmtInfs = _tenantDataContext.PtCmtInfs.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId);
            var ptHokenPatterns = _tenantDataContext.PtHokenPatterns.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId);
            var ptKohis = _tenantDataContext.PtKohis.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId);
            // Rsv (Reservation)
            var rsvInfs = _tenantDataContext.RsvInfs;
            var rsvFrameMsts = _tenantDataContext.RsvFrameMsts.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId);
            // User (Doctor)
            var userMsts = _tenantDataContext.UserMsts.Where(x => x.IsDeleted == DeleteTypes.None && x.HpId == hpId && x.JobCd == 1 && x.IsDeleted == 0);
            // Ka (Department)
            ...
            var models = raiins.Select(r => new ReceptionRowModel(
                ...
                dynamicCells: r.raiinKbnDetails.Select(d => new DynamicCell(d.GrpCd, d.KbnCd, d.KbnName, d.ColorCd ?? string.Empty)).ToList(),
                sinDate,
                r.relatedKaMst?.KaSname ?? string.Empty,
                r.relatedTanto?.Sname ?? String.Empty
            )).ToList();
