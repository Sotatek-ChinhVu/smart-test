    ...
    public class KensaSetRepository : RepositoryBase, IKensaSetRepository
    {
            ...
            executionStrategy.Execute(
                () =>
                {
                    ...
                    try
                    {
                        ...
                        int maxKensaSetSortNo = NoTrackingDataContext.KensaSets.Where(c => c.HpId == hpId).AsEnumerable().Select(c => c.SortNo).DefaultIfEmpty(0)?.Max() ?? 0;
                        // Create kensaSet
                        if (setId == 0)
                        {
                            maxKensaSetSortNo++;
                        ...
                        {
                            int maxKensaSetDetailSortNo = NoTrackingDataContext.KensaSetDetails.Where(c => c.HpId == hpId && c.SetId == setId).AsEnumerable().Select(c => c.SortNo).DefaultIfEmpty(0)?.Max() ?? 0;
        ...
        public List<KensaCmtMstModel> GetListKensaCmtMst(int hpId, int iraiCd, string keyword)
        {
            ...
            var kensaInKensaMst = from t1 in NoTrackingDataContext.KensaCmtMsts
                                  ...
                                  where t1.CmtSeqNo == NoTrackingDataContext.KensaCmtMsts.Where(m => m.HpId == hpId && m.CmtCd == t1.CmtCd && m.IsDeleted == DeleteTypes.None).Select(m => m.CmtSeqNo).DefaultIfEmpty().Min()
