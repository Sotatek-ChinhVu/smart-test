...
public class CoRaiinInfModel
{
    ...
    public CoRaiinInfModel(RaiinInf raiinInf, UserMst userMst)
    {
        RaiinInf = raiinInf;
        UserMst = userMst;
    }
    ...
    public string TantoName
    {
        get
        {
            ...
            if (UserMst != null)
            {
                if (!string.IsNullOrEmpty(UserMst.DrName))
                {
                    // 保険医氏名の設定があれば保険医氏名を返す
                    ret = UserMst.DrName;
                }
                else
                {
    ...
    {
        get { return UserMst == null ? "" : UserMst.MayakuLicenseNo ?? string.Empty; }
    }
}
