        ...
        CoSta9000RaiinConf? raiinConf, CoSta9000SinConf? sinConf, CoSta9000KarteConf? karteConf)
    {
        ...
        if (isSinConf)
        {
            ...
            if (sinConf?.SearchWord != string.Empty)
            {
                ...
                var keywordConditions = sinConf.SearchWord.Select(keyword => $"%{keyword}%").Distinct().ToList();
                if (sinConf?.WordOpt == 0)
                {
                    //or条件
                    sinJoins = sinJoins.Where(item => keywordConditions.Any(condition => EF.Functions.Like(item.ItemName, condition)));
                }
                else
                {
                    //and条件
                    sinJoins = sinJoins.Where(item => keywordConditions.All(condition => EF.Functions.Like(item.ItemName, condition)));
                }
            }
            //検索項目
