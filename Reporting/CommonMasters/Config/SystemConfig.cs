...
public class SystemConfig : RepositoryBase, ISystemConfig
{
    ...
    {
        _systemConfigs = NoTrackingDataContext.SystemConfs.Where(p => p.HpId == Session.HospitalID).ToList();
    }
    ...
    // 1: 削除患者の番号を再利用する
    /// </summary>
    public int EmptyPtIdSetting() { return (int)GetSettingValue(1014, 0); }

    public long AutoPtNumStartValue() { return (long)GetSettingValue(1014, 1); }

    public int IsFirstVisitTime() { return (int)GetSettingValue(1007, 0); }

    public int UketukeNoMode() { return (int)GetSettingValue(1008, 0); }

    public int UketukeNoStart() { return GetSettingParam(1008, defaultParam: "1").AsInteger(); }

    public int DefaultDoctorSetting() { return (int)GetSettingValue(1009, 0); }

    public int SameVisitSetting() { return (int)GetSettingValue(1010, 0); }

    public int FirstTimeConfirmDays() { return (int)GetSettingValue(1013, 0, 90); }
    /// <summary>
    /// 労災レセプト電算 0-なし、1-あり
    /// </summary>
    public int RosaiReceden() { return (int)GetSettingValue(100003, 0); }
    /// <summary>
    /// 労災レセプト電算 開始年月(yyyyMM)
    /// </summary>
    public string RosaiRecedenTerm() { return GetSettingParam(100003, 0); }

    public int IsRyoSyoDetail() { return (int)GetSettingValue(93002, 0); }

    public int PtNumCheckDigit() { return (int)GetSettingValue(1001); }

    public int FKanNmChkJIS() { return (int)GetSettingValue(1003); }

    public int TorokuJiYoyaku() { return (int)GetSettingValue(1004); }

    public int ExistPediatrics() { return (int)GetSettingValue(3004); }
    ...
    ///     1:労災準拠
    /// </summary>
    public int JibaiJunkyo() { return (int)GetSettingValue(3001, 0); }
    /// <summary>
    /// 自賠労災準拠加算率
    /// </summary>
    public double JibaiRousaiRate() { return GetSettingValue(3001, 1); }
    /// <summary>
    /// 多剤投与の臨時判断日数
    ///     既定値は14
    /// </summary>
    public int SyohoRinjiDays() { return (int)GetSettingValue(3002, 0, 14); }

    /// <summary>
    /// 自動入金
    ///     0:しない 1:する
    /// </summary>
    public int AutoNyukin() { return (int)GetSettingValue(3003, 0); }
    /// <summary>
    /// レセプト院外処方のコメント／用法出力
    ///     0:しない
    ///     1:する（既定値）
    /// </summary>
    public int OutDrugYohoDsp() { return (int)GetSettingValue(3005, 0, 1); }
    /// <summary>
    /// 処方料・調剤料・調基・処方箋料　自動算定時の保険組み合わせID
    ///     0:公費を優先
    ///     1:上位にオーダーされている内容に合わせる
    /// </summary>
    public int DrugPid() { return (int)GetSettingValue(3007, 0, 0); }

    public int HokenKohiKbn() { return (int)GetSettingValue(3006, 0); }
    ...
    ///     1:上限未満でも記載する
    /// </summary>
    public int ChokiTokki() { return (int)GetSettingValue(3006, 1, 0); }
    ...
    ///     3:公費負担額を含まない
    /// </summary>
    public int ChokiFutan() { return (int)GetSettingValue(3006, 2, 0); }
    ...
    /// </remarks>
    public int ChokiDateRange() { return (int)GetSettingValue(3006, 3, 0); }
    ...
    /// </summary>
    public int HoukatuHaihanCheckMode() { return (int)GetSettingValue(3008, 0, 0); }
    /// <summary>
    /// 包括背反ログ出力モード
    ///     0:すべて出力
    ///     1:自動発生項目は追加しない
    /// </summary>
    public int HoukatuHaihanLogputMode() { return (int)GetSettingValue(3009, 0, 0); }
    /// <summary>
    /// 包括背反ログで条件付きのログを出力するかどうか
    ///     0:出力しない
    ///     1:出力する
    /// </summary>  
    public int HoukatuHaihanSPJyokenLogputMode() { return (int)GetSettingValue(3009, 1, 0); }
    ...
    ///     9:記載しない
    /// </summary>
    public int ReceKyufuKisai() { return (int)GetSettingValue(3010, 0, 0); }
    ...
    ///     3:マル長の場合は記載しない
    /// </summary>
    public int ReceKyufuKisai2() { return (int)GetSettingValue(3010, 1, 0); }
    ...
    /// Paramに開始請求年月(YYYYMM)
    /// </summary>
    public int NaraFukusiReceCmt() { return (int)GetSettingValue(3011, 0, 0); }
    public string NaraFukusiReceCmtStartDate() { return GetSettingParam(3011, 0, ""); }
    ...
    ///     2:10円単位(切り捨て)
    /// </summary>
    public int RoundKogakuPtFutan() { return (int)GetSettingValue(3016, 0); }
    /// <summary>
    /// 検査まるめ分点（社保）
    /// 0 同一Rpにまとめる
    /// 1 別Rpに分ける
    /// </summary>
    public int KensaMarumeBuntenSyaho() { return (int)GetSettingValue(3017, 0); }
    /// <summary>
    /// 検査まるめ分点（国保）
    /// 0 同一Rpにまとめる
    /// 1 別Rpに分ける
    /// </summary>
    public int KensaMarumeBuntenKokuho() { return (int)GetSettingValue(3017, 1); }
    /// <summary>
    /// 自動発生コメント
    /// 0 算定情報優先
    /// 1 コメント区分優先(その他行為以外)
    /// </summary>
    public int CalcAutoComment() { return (int)GetSettingValue(3018, 0); }
    /// <summary>
    /// 検査重複チェックのログ出力
    /// 0 する
    /// 1 しない
    /// </summary>
    public int CalcCheckKensaDuplicateLog() { return (int)GetSettingValue(3019, 0); }
    /// <summary>
    /// レセプト院内処方の用法コメント出力
    /// </summary>
    public int InDrugYohoComment() { return (int)GetSettingValue(3022, 0, 0); }
    /// <summary>
    /// 訪問看護指導料が再診料を包括するかどうか
    /// 0 する
    /// 1 しない
    /// </summary>
    public int HoumonKangoSaisinHokatu() { return (int)GetSettingValue(3023, 0, 0); }
    /// <summary>
    /// 小児特定疾患カウンセリング料初回算定チェックを行うかどうか
    /// 0 初回算定日から2年まで
    /// 1 しない
    /// </summary>
    public int SyouniCounselingCheck() { return (int)GetSettingValue(3024, 0, 0); }
    public int AutoShowMonshinSheet() { return (int)GetSettingValue(2001, 0); }
    ...
    /// </summary>
    public int AutoUseHokenPatternFromHistory() { return (int)GetSettingValue(2008, 0); }
    ...
    /// 1: 保険パターンごとにチェックする
    /// </summary>
    public int HokenPatternExpiredCheckTiming() { return (int)GetSettingValue(2008, 2); }
    ...
    /// 1: 履歴で使用した自費算定を参考に決定
    /// </summary>
    public int AutoUseJihiSanteiFromHistory() { return (int)GetSettingValue(2008, 1); }
    ...
    /// 1: 院外
    /// </summary>
    public int DefaultInOutKensa() { return (int)GetSettingValue(2018, 0); }
    ...
    /// 2: 外注コード未設定
    /// </summary>
    public int CheckKensaIrai() { return (int)GetSettingValue(2019, 0); }
    ...
    /// 1: 検体（微生物検査除く）
    /// </summary>
    public int CheckKensaIraiCondition() { return (int)GetSettingValue(2019, 1); }
    ...
    /// 2: 一般名処方
    /// </summary>
    public int AutoSetSyohoKbnKohatuDrug() { return (int)GetSettingValue(2020, 0); }
    /// <summary>
    /// 後発品オーダー時の変更の制限
    /// 0: 制限なし
    /// 1: 剤型変更不可
    ...
    /// 3: 含量規格・剤型変更不可
    /// </summary>
    public int AutoSetSyohoLimitKohatuDrug() { return (int)GetSettingValue(2020, 1); }
    /// <summary>
    /// 後発品オーダー時の処方箋記載自動設定　セット優先区分
    /// 0: セットの設定を無視する
    /// 1: セットの設定を優先する
    /// </summary>
    public int AutoSetSyohoKisaiIgnoreSetKohatuDrug() { return (int)GetSettingValue(2020, 2); }
    /// <summary>
    ...
    /// 2: 一般名処方
    /// </summary>
    public int AutoSetSyohoKbnSenpatuDrug() { return (int)GetSettingValue(2021, 0); }
    /// <summary>
    ...
    /// 3: 含量規格・剤型変更不可
    /// </summary>
    public int AutoSetSyohoLimitSenpatuDrug() { return (int)GetSettingValue(2021, 1); }
    /// <summary>
    /// 後発品のある先発品オーダー時の処方箋記載自動設定　セット優先区分
    /// 0: セットの設定を無視する
    /// 1: セットの設定を優先する
    /// </summary>
    public int AutoSetSyohoKisaiIgnoreSetSenpatuDrug() { return (int)GetSettingValue(2021, 2); }

    public int FoodAllergyLevelSetting() { return (int)GetSettingValue(2027, 0, 3); }

    public int AgeLevelSetting() { return (int)GetSettingValue(2027, 3, 8); }

    public int AgeTypeCheckSetting() { return (int)GetSettingValue(2030, 0, 0); }

    public int CheckDupicatedSetting() { return (int)GetSettingValue(2027, 4, 1); }

    public int DiseaseLevelSetting() { return (int)GetSettingValue(2027, 2, 3); }

    public int KinkiLevelSetting() { return (int)GetSettingValue(2027, 1, 4); }

    public double DosageRatioSetting() { return GetSettingValue(2023, 0, 1); }

    public bool DosageMinCheckSetting() { return GetSettingValue(2023, 1, 1) != 0; }

    public bool DosageDrinkingDrugSetting() { return GetSettingValue(2023, 2, 1) != 0; }

    public bool DosageDrugAsOrderSetting() { return GetSettingValue(2023, 3, 1) != 0; }

    public bool DosageOtherDrugSetting() { return GetSettingValue(2023, 4, 1) != 0; }

    public int DoExcludeItemFromHistory() { return (int)GetSettingValue(2033, 0, 0); }
    ...
    /// </summary>
    public int SuspectedByomeiCheckMode() { return (int)GetSettingValue(2014, 0, 2); }
    ...
    /// </summary>
    public int DuplicatedByomeiCheckMode() { return (int)GetSettingValue(2015, 0, 2); }
    ...
    /// M 最終測定日から指定月後の初回来院時
    /// </summary>
    public int WeightConfirmDays() { return GetSettingParam(2010, 0).AsInteger(); }

    public int WeightCheckType() { return (int)GetSettingValue(2010, 0); }

    /// <summary>
    /// 0 : 小数点以下第2位で四捨五入
    /// 1～4 : 小数点以下第N位で四捨五入（Nは設定値）
    /// </summary>
    public int WeightRound() { return (int)GetSettingValue(2011, 0); }

    public bool IsDuplicatedComponentChecked() { return (int)GetSettingValue(2026, 0, 1) == 1; }

    public bool IsProDrugChecked() { return (int)GetSettingValue(2026, 1, 1) == 1; }

    public bool IsSameComponentChecked() { return (int)GetSettingValue(2026, 2, 1) == 1; }

    public bool IsDuplicatedClassChecked() { return (int)GetSettingValue(2026, 3, 1) == 1; }

    public bool IsDuplicatedComponentForDuplication() { return (int)GetSettingValue(2024, 0, 1) == 1; }

    public bool IsProDrugForDuplication() { return (int)GetSettingValue(2024, 1, 1) == 1; }

    public bool IsSameComponentForDuplication() { return (int)GetSettingValue(2024, 2, 1) == 1; }

    public bool IsDuplicatedClassForDuplication() { return (int)GetSettingValue(2024, 3, 1) == 1; }

    public int GetHaigouSetting() { return (int)GetSettingValue(2025, 0, 3); }

    public int ReceNoDspComment() { return (int)GetSettingValue(3012, 0, 0); }

    public int UpdateIgnoreOption() { return (int)GetSettingValue(8040, 0, 0); }

    public string SortPrinterSetting() { return GetSettingParam(93005, 0, defaultParam: "領収証,明細,院外処方箋,薬袋ラベル,薬情,お薬手帳シール"); }
    ...
    /// 1:使用する
    /// </summary>
    public int ApprovalFunc() { return (int)GetSettingValue(2022); }

    public string ApprovalFuncParam() { return GetSettingParam(2022); }

    public int PrDiInfClassification() { return (int)GetSettingValue(100037, 5, 0); }

    public int ApprovalNumberOfStartDateAgo() { return (int)GetSettingValue(2022, 1, 0); }
    ...
    ///     1:する
    /// </summary>
    public int JyusinHyoRpName() { return (int)GetSettingValue(91001, 1); }
    /// <summary>
    /// 受診票　検査容器/材料情報印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int JyusinHyoKensaYokiZairyo() { return (int)GetSettingValue(91001, 2); }
    /// <summary>
    /// 受診票　印刷対象来院区分設定
    /// </summary>
    public string JyusinHyoRaiinKbn() { return GetSettingParam(91001, 3); }
    /// <summary>
    /// 受診票　患者コメント
    ///     0:印字する
    ///     1:印字しない
    /// </summary>
    public int JyusinHyoPtCmt() { return (int)GetSettingValue(91001, 4); }
    /// <summary>
    /// 受診票　アレルギー情報
    ///     0:印字する
    ///     1:印字しない
    /// </summary>
    public int JyusinHyoAlrgy() { return (int)GetSettingValue(91001, 5); }
    /// <summary>
    /// オーダーラベル　予約オーダー印字
    ...
    ///     2:印字しない
    /// </summary>
    public int OrderLabelYoyakuOrderDsp() { return (int)GetSettingValue(92001, 1); }
    /// <summary>
    /// オーダーラベル　検査項目表示
    ///     0:1行複数項目
    ///     1:1行1項目
    /// </summary>
    public int OrderLabelKensaDsp() { return (int)GetSettingValue(92001, 3); }
    /// <summary>
    /// オーダーラベル　次回予約日の印字
    ///     0:印字しない
    ///     1:印字する
    /// </summary>
    public int OrderLabelYoyakuDateDsp() { return (int)GetSettingValue(92001, 4); }
    /// <summary>
    /// オーダーラベル　ヘッダー印字パターン
    ///     0:1印刷ごとに1つ印字
    ///     1:Rpごとに1つ印字
    /// </summary>
    public int OrderLabelHeaderPrint() { return (int)GetSettingValue(92001, 5); }
    /// <summary>
    /// オーダーラベル　算定外マーク表示
    ///     0:表示しない
    ///     1:表示する
    /// </summary>
    public int OrderLabelSanteiGaiDsp() { return (int)GetSettingValue(92001, 6); }
    /// <summary>
    /// オーダーラベル　診療科の印字
    /// 　　0:印字しない
    /// 　　1:印字する
    /// </summary>
    public int OrderLabelKaPrint() { return (int)GetSettingValue(92001, 7); }
    /// <summary>
    /// オーダーラベル　初再診の印字
    /// 　　0:印字しない
    /// 　　1:印字する
    /// </summary>
    public int OrderLabelSyosaiPrint() { return (int)GetSettingValue(92001, 8); }
    /// <summary>
    /// オーダーラベル　入力者の印字
    /// 　　0:印字しない
    /// 　　1:印字する
    /// </summary>
    public int OrderLabelCreateNamePrint() { return (int)GetSettingValue(92001, 10); }

    public int OrderLabelOdrKouiKbn() { return (int)GetSettingValue(92001, 11); }
    /// <summary>
    /// 院外処方箋　臨時のRpに臨時のコメントを自動記載するかどうか
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SyohosenRinjiKisai() { return (int)GetSettingValue(92003, 1); }
    /// <summary>
    /// 院外処方箋　地域包括診療料等算定時、処方箋備考欄にコメント自動印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SyohosenChiikiHoukatu() { return (int)GetSettingValue(92003, 2); }
    /// <summary>
    /// 院外処方箋　QRコード印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SyohosenQRKbn() { return (int)GetSettingValue(92003, 3); }
    /// <summary>
    /// 院外処方箋　処方箋単位
    ///     0:オーダー単位
    ///     1:レセ単位
    /// </summary>
    public int SyohosenTani() { return (int)GetSettingValue(92003, 4); }
    /// <summary>
    /// 院外処方箋　控え印刷
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SyohosenHikae() { return (int)GetSettingValue(92003, 5); }
    /// <summary>
    /// 院外処方箋　負担率印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SyohosenFutanRate() { return (int)GetSettingValue(92003, 6); }
    /// <summary>
    /// 院外処方箋　QRコードのバージョン
    ///     0:JAHIS5
    ///     1:JAHIS7
    ...
    ///     3:JAHIS9
    /// </summary>
    public int SyohosenQRVersion() { return (int)GetSettingValue(92003, 7); }
    /// <summary>
    /// リフィル処方0回の時の印字文字
    /// </summary>
    public string SyohosenRefillZero() { return GetSettingParam(92003, 9); }
    /// <summary>
    /// リフィル処方でない場合のリフィル可欄の取り繕い
    ///     1:取り消し線を引く
    /// </summary>
    public int SyohosenRefillStrikeLine() { return (int)GetSettingValue(92003, 9); }
    /// <summary>
    /// 指示箋　Rp名称印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SijisenRpName() { return (int)GetSettingValue(92008, 1); }
    /// <summary>
    /// 指示箋　検査容器/材料情報印字
    ///     0:しない
    ///     1:する
    /// </summary>
    public int SijisenKensaYokiZairyo() { return (int)GetSettingValue(92008, 2); }
    /// <summary>
    /// 指示箋　患者コメント
    ///     0:印字する
    ///     1:印字しない
    /// </summary>
    public int SijisenPtCmt() { return (int)GetSettingValue(92008, 3); }
    /// <summary>
    /// 指示箋　アレルギー情報
    ///     0:印字する
    ///     1:印字しない
    /// </summary>
    public int SijisenAlrgy() { return (int)GetSettingValue(92008, 4); }

    public int SijisenCheckMachine() { return (int)GetSettingValue(92008, 5); }

    public string SijisenCheckMachineParam() { return GetSettingParam(92008, 5, "KrtRenkei,TKImport"); }
    /// <summary>
    /// 領収証タイプ
    /// </summary>
    public int AccountingFormType() { return (int)GetSettingValue(93001, 1); }
    /// <summary>
    /// 月間領収証タイプ
    /// </summary>
    public int AccountingMonthFormType() { return (int)GetSettingValue(93001, 2); }
    /// <summary>
    /// 領収証明細タイプ
    /// </summary>
    public int AccountingDetailFormType() { return (int)GetSettingValue(93002, 1); }
    /// <summary>
    /// 月間領収証明細タイプ
    /// </summary>
    public int AccountingDetailMonthFormType() { return (int)GetSettingValue(93002, 2); }
    /// <summary>
    /// 領収証明細　院外処方印字
    ///     0:印字しない　1:印字する
    /// </summary>
    public int AccountingDetailIncludeOutDrug() { return (int)GetSettingValue(93002, 4); }
    /// <summary>
    /// 領収証明細　コメント印字
    ///     0:印字しない　1:印字する
    /// </summary>
    public int AccountingDetailIncludeComment() { return (int)GetSettingValue(93002, 5); }
    /// <summary>
    /// 下位互換フィールドの使用
    ///     0:使用しない　1:使用する（既定値）
    /// </summary>
    public int AccountingUseBackwardFields() { return (int)GetSettingValue(93003, 0, 1); }
    /// <summary>
    /// 領収証定型文
    ///     0:印字しない　1:印字する
    /// </summary>
    public int AccountingTeikeibunPrint() { return (int)GetSettingValue(93004, 0); }
    /// <summary>
    /// 領収証定型文 1行目
    /// </summary>
    public string AccountingTeikeibun1() { return GetSettingParam(93004, 1); }
    /// <summary>
    /// 領収証定型文 2行目
    /// </summary>
    public string AccountingTeikeibun2() { return GetSettingParam(93004, 2); }
    /// <summary>
    /// 領収証定型文 3行目
    /// </summary>
    public string AccountingTeikeibun3() { return GetSettingParam(93004, 3); }
    /// <summary>
    /// 光ディスク送付書備考欄
    ///     0:総件数を記載しない
    ///     1:総件数を記載する
    /// </summary>
    public int HikariDiskIsTotalCnt() { return (int)GetSettingValue(94004, 0); }
    /// <summary>
    /// レセプト病名記載
    ///     0: 1行複数
    ///     1: 1行1病名
    /// </summary>
    public int ReceiptByomeiWordWrap() { return (int)GetSettingValue(94001, 0); }
    /// <summary>
    /// レセプト病名転帰日記載
    ///     0: 記載しない
    ///     1: 記載する
    /// </summary>
    public int ReceiptByomeiTenkiDate() { return (int)GetSettingValue(94001, 1); }
    /// <summary>
    /// 労災レセプト様式
    ///     0:旧様式
    ///     1:新様式(2019/05~）
    /// </summary>
    public int RousaiReceiptType() { return (int)GetSettingValue(94002, 0); }
    /// <summary>
    /// 東京都国保総括表用紙タイプ
    ///     0:専用紙
    ///     1:オーバーレイ
    /// </summary>
    public int P13KokhoSokatuType() { return (int)GetSettingValue(94003, 0); }
    /// <summary>
    /// 東京都負担医療費請求書用紙タイプ
    ///     0:専用紙
    ///     1:オーバーレイ
    /// </summary>
    public int P13WelfareGreenSeikyuType() { return (int)GetSettingValue(94003, 2); }
    /// <summary>
    /// 東京都難病医療費請求書用紙タイプ
    ///     0:専用紙
    ///     1:オーバーレイ
    /// </summary>
    public int P13WelfareBlueSeikyuType() { return (int)GetSettingValue(94003, 3); }
    /// <summary>
    /// 小児喘息レセプトタイプ
    ///     0:大阪市タイプ
    ///     1:東大阪市タイプ
    /// </summary>
    public int SyouniZensokuType() { return (int)GetSettingValue(94005, 0); }
    /// <summary>
    /// レセプト電算院外処方診療区分タイプ
    ///     0: 2x
    ///     1: Zx
    /// </summary>
    public int ReceiptOutDrgSinId() { return (int)GetSettingValue(94006, 0); }
    /// <summary>
    /// レセプトコメントのみのRpに点数×回数を記載するか
    ///     0: 記載しない
    ///     1: 記載する
    /// </summary>
    public int ReceiptCommentTenCount() { return (int)GetSettingValue(94007, 0); }
    /// <summary>
    /// 病名条件
    /// </summary>
    ...
    /// ネームラベル
    /// </summary>
    public int NameLableConfig() { return (int)GetSettingValue(91006, 0); }
    ...
    public string InnaishohosenCheckMachineParam() { return GetSettingParam(92002, 2, "KrtRenkei,TKImport"); }
    ...
    /// 薬袋ラベル（内服／用紙小・プリンタ）
    /// </summary>
    public string YakutaiNaifukuPaperSmallPrinter() { return GetSettingParam(92005, 11); }
    /// <summary>
    /// 薬袋ラベル（内服／用紙中・最小服用量）
    /// </summary>
    ...
    /// 薬袋ラベル（内服／用紙中・プリンタ）
    /// </summary>
    public string YakutaiNaifukuPaperNormalPrinter() { return GetSettingParam(92005, 12); }
    /// <summary>
    /// 薬袋ラベル（内服／用紙大・最小服用量）
    /// </summary>
    ...
    /// 薬袋ラベル（内服／用紙大・プリンタ）
    /// </summary>
    public string YakutaiNaifukuPaperBigPrinter() { return GetSettingParam(92005, 13); }
    /// <summary>
    /// （頓服／用紙小）
    /// </summary>
    ...
    /// （頓服／用紙小・プリンタ）
    /// </summary>
    public string YakutaiTonpukuPaperSmallPrinter() { return GetSettingParam(92005, 21); }
    /// <summary>
    /// （頓服／用紙中・最小服用量）
    /// </summary>
    ...
    /// （頓服／用紙中・プリンタ）
    /// </summary>
    public string YakutaiTonpukuPaperNormalPrinter() { return GetSettingParam(92005, 22); }
    /// <summary>
    /// （頓服／用紙大・最小服用量）
    /// </summary>
    ...
    /// （頓服／用紙大・プリンタ）
    /// </summary>
    public string YakutaiTonpukuPaperBigPrinter() { return GetSettingParam(92005, 23); }
    /// <summary>
    /// （外用／用紙小）
    /// </summary>
    ...
    /// （外用／用紙小・プリンタ）
    /// </summary>
    public string YakutaiGaiyoPaperSmallPrinter() { return GetSettingParam(92005, 31); }
    /// <summary>
    /// （外用／用紙中・最小服用量）
    /// </summary>
    ...
    /// （外用／用紙中・プリンタ）
    /// </summary>
    public string YakutaiGaiyoPaperNormalPrinter() { return GetSettingParam(92005, 32); }
    /// <summary>
    /// （外用／用紙大・最小服用量）
    /// </summary>
    ...
    /// （外用／用紙大・プリンタ）
    /// </summary>
    public string YakutaiGaiyoPaperBigPrinter() { return GetSettingParam(92005, 33); }
    /// <summary>
    /// 薬袋ラベル用紙サイズ  
    /// </summary>
    ...
    /// 服用時点別一包化指示項目
    /// </summary>
    public string YakutaiFukuyojiIppokaItemCd() { return GetSettingParam(92005, 40); }
    /// <summary>
    /// 薬情 92004
    /// </summary>
    public int Kusurijo => (int)GetSettingValue(92004);
    public int KusurijoCheckMachine => (int)GetSettingValue(92004, 16);
    public string KusurijoCheckMachineParam() { return GetSettingParam(92004, 16, "KrtRenkei,TKImport"); }
    ...
    public string IngaiShohosenCheckMachineParam() { return GetSettingParam(92003, 8, "KrtRenkei,TKImport"); }
    ...
    public string OrderLabelCheckMachineParam() { return GetSettingParam(92001, 9, "KrtRenkei,TKImport"); }
    ...
    /// レセプト確認画面の総点数表示
    /// </summary>
    public int ReceiptListTensuCountDisplay() { return (int)GetSettingValue(6001, 0); }
    /// <summary>
    /// レセプト診療科検索
    /// 0-請求点数が最も高い診療科を検索する, 1-来院があった診療科を検索する
    /// </summary>
    public int ReceiptKaIdTarget() { return (int)GetSettingValue(6002, 0); }
    /// <summary>
    /// レセプト担当医検索
    /// 0-請求点数が最も高い担当医を検索する, 1-来院があった担当医を検索する
    /// </summary>
    public int ReceiptTantoIdTarget() { return (int)GetSettingValue(6002, 1); }
    /// <summary>
    ...
    /// グループID,グループコード
    /// </summary>
    public string TeamKarteTargetGrpId() { return GetSettingParam(100021, 4); }
    /// <summary>
    /// チームカルテ　ファイル保存先
    /// </summary>
    public string TeamKarteFilePath() { return GetSettingParam(100021, 5); }
    /// <summary>
    /// チームカルテ　訪問施設グループID
    /// </summary>
    ...
    /// </summary>
    public string TeamKarteFinishRaiinKbnCd() { return GetSettingParam(100021, 7); }
    /// <summary>
    /// チームカルテ　検歴連携タイプ
    ...
    /// </summary>
    public string TeamKarteKenrekiLastDate()
    {
    ...
    /// チームカルテ　FTPサーバー
    /// </summary>
    public string TeamKarteFTPServer()
    {
    ...
    /// チームカルテ　FTPユーザー名
    /// </summary>
    public string TeamKarteFTPUser()
    {
    ...
    /// チームカルテ　FTPパスワード
    /// </summary>
    public string TeamKarteFTPPassword()
    {
    ...
    /// チームカルテ　メールサーバー
    /// </summary>
    public string TeamKarteMailServer()
    {
    ...
    /// </summary>
    public string OdrkensaIraiKaCodeParam()
    {
    ...
    /// Planet センターコード
    /// </summary>
    public string PlanetCenterCode()
    {
    ...
    /// </summary>
    public string PlanetVitalTemplateHostName()
    {
    ...
    /// </summary>
    public string PlanetVitalTemplateDatabase()
    {
    ...
    /// </summary>
    public string PlanetVitalTemplateUserName()
    {
    ...
    /// </summary>
    public string PlanetVitalTemplatePassword()
    {
    ...
    /// </summary>
    public string PlanetVitalTemplateCenterCode()
    {
    ...
    /// </summary>
    public string PlanetVitalPhysicalHostName()
    {
    ...
    /// </summary>
    public string PlanetVitalPhysicalDatabase()
    {
    ...
    /// </summary>
    public string PlanetVitalPhysicalUserName()
    {
    ...
    /// </summary>
    public string PlanetVitalPhysicalPassword()
    {
    ...
    /// </summary>
    public string PlanetVitalPhysicalCenterCode()
    {
    ...
    /// 労災レセプト電算開始年月
    /// </summary>
    public string RousaiRecedenStartYm()
    {
    ...
    /// アフターケアレセプト電算開始年月
    /// </summary>
    public string AfterCareRecedenStartYm()
    {
    ...
    /// 経過日数
    /// </summary>
    public string NumDaysFromLastVisit()
    {
    ...
    /// プチ予約連携ライセンス
    /// </summary>
    public string PetitReservationParam()
    {
    ...
    public string PrintDrgLabelCheckMachineParam()
    {
    ...
    public string PrintDrgNoteCheckMachineParam()
    {
    ...
    /// コニカUser
    /// </summary>
    public string KonikaUserSetting()
    {
    ...
    /// コニカPassword
    /// </summary>
    public string KonikaPasswordSetting()
    {
    ...
    public string StandbyServerParam()
    {
    ...
    public string RenkeiYoyakuParam()
    {
    ...
    public string RenkeiTemplateParam()
    {
    ...
    /// アフターケア電算 0-なし、1-あり
    /// </summary>
    public int AftercareDensanLicense() { return (int)GetSettingValue(100003, 1); }

    /// <summary>
    /// アフターケア電算 開始年月(yyyyMM)
    /// </summary>
    public string AftercareDensanTerm() { return GetSettingParam(100003, 1); }

    /// <summary>
    /// 至急
    /// </summary>
    public bool IsShowUrgentDetail() { return GetSettingValue(2029, 0) == 1; }
    /// <summary>
    /// 請求情報連携（窓口）　入金額0連携
    /// 0-しない、1-する
    /// </summary>
    public int SeikyuRenkeiMadoguchi() { return (int)GetSettingValue(100031, 0); }
    /// <summary>
    /// 請求情報連携（収納）　入金額0連携
    /// 0-しない、1-する
    /// </summary>
    public int SeikyuRenkeiSyuno() { return (int)GetSettingValue(100031, 1); }
    /// <summary>
    /// 請求情報連携　来院番号が異なる場合
    /// 0-同一ファイルに出力、1-別ファイルに出力
    /// </summary>
    public int SeikyuRenkeiDiffRaiinNo() { return (int)GetSettingValue(100031, 2); }
    /// <summary>
    /// アットリンク　クライアントID
    /// </summary>
    public string AtLinkClientId() { return GetSettingParam(100032, 0); }

    public bool IsEnableRaiinView() { return (int)GetSettingValue(100030, 0) == 1; }

    // 注意事項に記載する内容
    /// <summary>
    /// 年齢条件かつ性別条件のあるもの
    /// </summary>
    public bool IsPrecautionQueryAgeSex() { return (int)GetSettingValue(92004, 3) == 1; }
    /// <summary>
    /// （年齢条件のみがあるもの）
    /// </summary>
    public bool IsPrecautionQueryAgeNoSex() { return (int)GetSettingValue(92004, 4) == 1; }
    /// <summary>
    /// （性別条件のみがあるもの）
    /// </summary>
    public bool IsPrecautionQuerySexNoAge() { return (int)GetSettingValue(92004, 5) == 1; }
    /// <summary>
    /// （年齢及び性別の条件がないもの）
    /// </summary>
    public bool IsPrecautionQueryNoAgeNoSex() { return (int)GetSettingValue(92004, 6) == 1; }
    /// <summary>
    /// （服用方法、使用方法）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd1() { return (int)GetSettingValue(92004, 7) == 1; }
    /// <summary>
    /// （飲食物および嗜好品の指示、指導、注意）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd2() { return (int)GetSettingValue(92004, 8) == 1; }
    /// <summary>
    /// （生活上の指示、指導、注意）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd3() { return (int)GetSettingValue(92004, 9) == 1; }
    /// <summary>
    /// （相互作用に関する注意）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd4() { return (int)GetSettingValue(92004, 10) == 1; }
    /// <summary>
    /// （既往症･体質、治療中の疾患）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd5() { return (int)GetSettingValue(92004, 11) == 1; }
    /// <summary>
    /// （妊娠および授乳）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd6() { return (int)GetSettingValue(92004, 12) == 1; }
    /// <summary>
    /// （保管方法）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd7() { return (int)GetSettingValue(92004, 13) == 1; }
    /// <summary>
    /// （臨床検査）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd8() { return (int)GetSettingValue(92004, 14) == 1; }
    /// <summary>
    /// （薬の作用に関わる注意）
    /// </summary>
    public bool IsPrecautionQueryPropertyCd9() { return (int)GetSettingValue(92004, 15) == 1; }

    /// <summary>
    /// 日計表の検索条件「入金時間」
    /// </summary>
    public bool IsShowPaymentTimeDailyStatistic() { return (int)GetSettingValue(95001, 0, 0) == 1; }

    public bool IsAutoEnableVisitingDataGrid() { return (int)GetSettingValue(5004, 0) == 1; }

    public bool IsCaptureScreenEnable() { return (int)GetSettingValue(2031, 0) == 1; }

    public bool IsCaptureScreenWithByomeiEnabled() { return (int)GetSettingValue(2032, 0) == 1; }

    /// <summary>
    /// ユーザー名
    /// </summary>
    public string CloudUserName() { return GetSettingParam(8031, 0) + "Emr"; }

    public string CloudPassword() { return "cbk#Emr" + GetSettingParam(8031, 0); }

    public int OdrItemNameDisplay() { return (int)GetSettingValue(100017, 4); }

    public int OdrJikarDate() { return (int)GetSettingValue(100017, 1); }

    public int OdrJikarCondition() { return (int)GetSettingValue(100017, 3); }

    public int HokenJikarCondition() { return (int)GetSettingValue(100017, 13); }
    ...
    public int ByomeiOrderFromSuperset => (int)GetSettingValue(2034, 0);
    ...
    /// 登美ヶ丘モール連携　総合受付ホスト
    /// </summary>
    public string MallRenkeiSogoHost()
    {
    ...
    /// MWM連携　対象項目コード
    /// </summary>
    public string MWMTargetItemCd()
    {
        return GetSettingParam(100038, 0);
    }

    public int DrugInfReportType => (int)GetSettingValue(92004, 1); // 0,1 - 1 Pic; 2 - 2 Pics; 3- No Pic

    public int DrugInfSelectedFormType => (int)GetSettingValue(92004, 17); //0 - Type 1; 1 - Type 2

    public int PrintDrugCommentSetting => (int)GetSettingValue(92004, 18); // 0 - don't print drug cmt; 1 - pint drug cmt

    public int ConfigIniFolder => (int)GetSettingValue(100039, 0); //0 - machine resolution 1; 1 - machine name
