            ...
            foreach (CoOdrInfModel odrInf in odrInfs.OrderBy(p => p.OdrKouiKbn).ThenBy(p => p.SortNo).ThenBy(p => p.RpNo).ThenBy(p => p.RpEdaNo))
            {
                ...
                if (ippo)
                {
                    ...
                    for (int i = 0; i < fukuyojis.Count; i++)
                    {
                        ...
                        {
                            // 服用時設定別にOdrInfを生成
                            long rpEdaNo = odrInfs.Where(p => p.RpNo == odrInf.RpNo).Select(p => p.RpEdaNo).DefaultIfEmpty()?.Max() ?? 0;

                            if (appendOdrInfs.Any(p => p.RpNo == odrInf.RpNo))
                            {
                                long rpEdaNo2 = appendOdrInfs.Where(p => p.RpNo == odrInf.RpNo).Select(p => p.RpEdaNo).DefaultIfEmpty()?.Max() ?? 0;
