...
{
    public class P24WelfareSofuDiskCoReportService : IP24WelfareSofuDiskCoReportService
    {
        #region Constant
        ...
        private string _formFileName = "p24WelfareSofuDisk.rse";

        #region Constructor and Init
        public P24WelfareSofuDiskCoReportService(ICoWelfareSeikyuFinder welfareFinder)
        {
        ...
        public CommonReportingRequestModel GetP24WelfareSofuDiskReportingData(int hpId, int seikyuYm, SeikyuType seikyuType, int diskCnt)
        {
            try
            {
                this.hpId = hpId;
        ...
        private bool UpdateDrawForm()
        {
            ...
            int UpdateFormBody()
            {
                ...
                //医療機関コード
                fieldDataPerPage.Add("hpCode", string.Format("241{0, 7}", hpInf.HpCd));
                //提出枚数
                fieldDataPerPage.Add("diskCnt", diskCnt.ToString());
                ...
                for (short rowNo = 0; rowNo < maxRow; rowNo++)
                {
                    ...
                    switch (rowNo)
                    {
                        case 5: count = receInfs.Count(); break;
                        default: count = receInfs.Where(r => r.KohiSbt == rowNo + 1).Count(); break;
                    }
