...
namespace CommonCheckers.OrderRealtimeChecker.Services
{
        ...
        where TOdrDetail : class, IOdrInfoDetailModel
    {
        ...
        public override UnitCheckerForOrderListResult<TOdrInf, TOdrDetail> HandleCheckOrderList(UnitCheckerForOrderListResult<TOdrInf, TOdrDetail> unitCheckerForOrderListResult)
        {
            ...
            List<DiseaseResultModel> checkedResultForCurrentDisease = Finder!.CheckContraindicationForCurrentDisease(HpID, PtID, settingLevel, Sinday, listItemCode, unitCheckerForOrderListResult.PtDiseaseModels);
            if (checkedResultForCurrentDisease != null)
            {
                checkedResult.AddRange(checkedResultForCurrentDisease);
            }

            List<DiseaseResultModel> checkedResultForHistoryDisease = Finder.CheckContraindicationForHistoryDisease(HpID, PtID, settingLevel, Sinday, listItemCode, unitCheckerForOrderListResult.SpecialNoteModel.ImportantNoteModel.KioRekiItems);
            if (checkedResultForHistoryDisease != null)
            {
                checkedResult.AddRange(checkedResultForHistoryDisease);
            }

            List<DiseaseResultModel> checkedResultForFamilyDisease = Finder.CheckContraindicationForFamilyDisease(HpID, PtID, settingLevel, Sinday, listItemCode, unitCheckerForOrderListResult.FamilyModels);
