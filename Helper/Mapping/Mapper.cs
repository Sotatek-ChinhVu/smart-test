...
namespace Helper.Mapping
{
    public static class Mapper
    {
        public class MapSchema<F, T> //where T : class ,new()
        {
            ...
            public T Map(F from, T to)
            {
                ...
                if (reviseSourcePropsFunc != null)
                {
                    var revisedProperties = (from item in fromProperties
                                             select
                                                 new PropertyItem(item, reviseSourcePropsFunc.Invoke(item.Name))
                                            ).ToArray();

                    foreach (var toProp in toProperties)
                    {
                        var propName = toProp.Name;
                        var fromProp = revisedProperties.FirstOrDefault(s => s.PropertyName == propName) ?? new PropertyItem(null,string.Empty);
                        CustomAttributeData? fromAttr = null;
                        if(fromProp != null && fromProp.PropertyInfo != null)
                            fromProp.PropertyInfo.CustomAttributes.FirstOrDefault(m => m.AttributeType == typeof(EntityIgnoreMappingAttribute));
                        var toAttr = toProp.CustomAttributes.FirstOrDefault(m => m.AttributeType == typeof(EntityIgnoreMappingAttribute));
                        ...
                        {
                            var fromValue = fromProp.PropertyInfo == null ? null : fromProp.PropertyInfo.GetValue(@from, null);
                ...
                foreach (var toProp in toProperties)
                {
                    ...
                    var fromAttr = fromProp.CustomAttributes.FirstOrDefault(m => m.AttributeType == typeof(EntityIgnoreMappingAttribute));
                    var toAttr = toProp.CustomAttributes.FirstOrDefault(m => m.AttributeType == typeof(EntityIgnoreMappingAttribute));
                    if (fromAttr != null || toAttr != null)
                    {
                        continue;
                    ...
                    if (CanWrite(toProp))
                    {
                        ...
                        try
                        {
                            if (toProp.PropertyType.Name.Equals("List`1") || (toProp.PropertyType.BaseType != null && toProp.PropertyType.BaseType.Name.Equals("Array")))
                            {
                                continue;
                            }
                            ...
                            else if (fromValue == null && toProp.PropertyType.Name.Equals("DateTime"))
                            {
                                continue;
                            }
                            else
            ...
            public F MapBack(T from, F to)
            {
                ...
                if (reviseSourcePropsFunc != null)
                {
                    ...
                    foreach (var fromProp in fromProperties)
                    {
                        ...
                        if (toProp != null && (toProp.PropertyInfo != null && CanWrite(toProp.PropertyInfo)))
                        {
                            var fromValue = fromProp.GetValue(@from, null);
            ...
            {
                public PropertyItem(PropertyInfo? propertyInfo, string propertyName)
                {
        ...
        /// <returns></returns>
        public static T Map<F, T>(F? from, T to, Func<F, T, T>? func = null) where T : class where F : class
        {
        ...
        /// <param name="func"></param>
        /// <returns></returns>
        public static List<T> Map<F, T>(IEnumerable<F> from, Func<F, T, T>? func = null) where T : class, new() where F : class
        {
            var lst = new List<T>();
