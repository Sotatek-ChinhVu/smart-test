...
namespace EmrCloudApi.Tenant.Responses.OrdInfs
{
    ...
    {
        public GetHeaderInfResponse(int syosaiKbn, int jikanKbn, int hokenPid, int santeiKbn, int tantoId, int kaId, string uketukeTime, string sinStartTime, string sinEndTime, OrdInfModel odrInfs)
        {
            SyosaiKbn = syosaiKbn;
            JikanKbn = jikanKbn;
            HokenPid = hokenPid;
