...
namespace EmrCloudApi.Tenant.Presenters.NextOrder
{
    public class UpsertNextOrderListPresenter : IUpsertNextOrderListOutputPort
    {
        ...
        {
            var validationOrders = new List<UpsertOrderInfItemResponse>();
            var validationKartes = new List<UpsertKarteInfItemResponse>();
            var validationNextOrders = new List<UpsertNextOrderItemResponse>();
            var validationByomeis = new List<UpsertByomeiItemResponse>();
            ...
            if (outputData.ValidationOdrs.Any())
            {
                foreach (var validation in outputData.ValidationOdrs)
                {
                    ...
                    validationOrders.Add(new UpsertOrderInfItemResponse(validation.Item1, dictionaryOneNextOrder));
                }
            }
            ...
            foreach (var validation in outputData.ValidationKarte)
            {
                ...
                validationKartes.Add(new UpsertKarteInfItemResponse(validation.Item1, validationKarte));
            }
            ...
            {
                var validationByomeisOfOne = new UpsertByomeiItemResponse(validation.Key, validation.Select(v => new UpsertByomeiValidationItemResponse(v.Item3, v.Item2, ConvertUpsertByomeiStatusToMessage(v.Item3))).ToList());
