        ...
        public void Complete(GetInsuranceMasterDetailOutputData output)
        {
            Result = new Response<GetInsuranceMasterDetailResponse>()
            {
                Data = new GetInsuranceMasterDetailResponse(
                    output.InsuranceMstData.Select(x => new InsuranceMasterDto(x.Master, x.Details.Select(m => new InsuranceDetailDto(m.HokenEdaNo, m.SortNo, m.HokenSName, m.PrefNo, m.HokenEdaNo, m.StartDate, m.HokenNo, false))))),
