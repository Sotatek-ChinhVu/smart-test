...
namespace EmrCloudApi.Controllers
{
    ...
    public class MedicalExaminationController : AuthorizeControllerBase
    {
        ...
        {
            var input = new GetOrderCheckerInputData(request.PtId, request.HpId, request.SinDay, request.CurrentListOdr, request.ListCheckingOrder, request.SpecialNoteItem, request.PtDiseaseModels, request.FamilyItems);
            var output = _bus.Handle(input);
