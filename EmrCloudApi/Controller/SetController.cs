...
public class SetController : AuthorizeControllerBase
{
    ...
    {
        var input = new CopyPasteSetMstInputData(HpId, UserId, request.CopySetCd, request.PasteSetCd, request.PasteToOtherGroup, request.PasteSetKbnEdaNo, request.PasteSetKbn);
        var output = _bus.Handle(input);
